...
class OutputGuessesEasyLevel : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_output_guesses)


        //Appends all the information inputted and outputs the results on this activity
        var fileInputStream: FileInputStream? = null
        fileInputStream = openFileInput("EasyMode_guesses.txt") //directs to this specific activty and sends all the data
        ...
        btnClear2.setOnClickListener {
            try {
                val file = File(applicationContext.filesDir, "EasyMode_guesses.txt")
