...
import kotlinx.android.synthetic.main.activity_easy_level.btnNext
import kotlinx.android.synthetic.main.activity_easy_level.btnSubmit
import kotlinx.android.synthetic.main.activity_easy_level.editTextNumber
import java.io.File
...
class HardLevel : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSubmit.setOnClickListener {
            ...
            while (editTextNumber.text.toString().toInt() != randomnumber && chances <= chances) {
                if (editTextNumber.text.toString()
                        .toInt() == randomnumber
                ) {//if guessing number is equal to random number
                    Toast.makeText(
                        this,
                        "Congrats the Number was " + randomnumber + "!" +
                                "Click on the next button to go next page.", Toast.LENGTH_LONG
                    ).show()
                    ...
                    val fileHardMode = File(applicationContext.filesDir, "HardMode_guesses.txt")
                    val file2 = File(applicationContext.filesDir, "PastGuesses.txt")

                    //constructing the file in a way to append the data to it.
                    try {
                        FileOutputStream(fileHardMode, true).bufferedWriter().use { writer ->
                            writer.append(
                    ...
                    Toast.makeText(this, "Submit a guess please", Toast.LENGTH_LONG).show()
                    break;
                    ...
                    Toast.makeText(
                        this, "This round is over! The guessing number was " + randomnumber + "! " +
                                "Click on the button to go to the next page.", Toast.LENGTH_LONG
                    ).show()
                    ...
                    Toast.makeText(
                        this,
                        "Number might or may not be smaller. Try another number. " +
                    ...
                    Toast.makeText(
                        this,
                        "Number might be smaller. Try another number." +
                                "you have " + chances + " chances left.",
                        Toast.LENGTH_LONG
                    ).show()
                    editTextNumber.text.toString() == ""
        ...
        btnNext.setOnClickListener {//goes to the next activity demonstrated
            val newIntent = Intent(this, OutputGuessesHardLevel::class.java)
