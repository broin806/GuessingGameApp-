...
class EasyLevel : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) { //upon onCreate
        super.onCreate(savedInstanceState)

        setContentView(R.layout.activity_easy_level)

        btnNext.visibility = View.INVISIBLE //set next button the invisible
        ...
        btnSubmit.setOnClickListener {
            ...
            while (editTextNumber.text.toString().toInt() != randomnumber && chances <= chances) {
                ...
                if (editTextNumber.text.toString().toInt() == randomnumber
                    ...
                    val fileEasyMode = File(applicationContext.filesDir, "EasyMode_guesses.txt")
                    val file2 = File(applicationContext.filesDir, "PastGuesses.txt")

                    //constructing the file in a way to append the data to it.
                    try {
                        FileOutputStream(fileEasyMode, true).bufferedWriter().use { writer ->
                            writer.append(
        ...
        btnNext.setOnClickListener {//goes to the next activity demonstrated
            val newIntent = Intent(this, OutputGuessesEasyLevel::class.java)
